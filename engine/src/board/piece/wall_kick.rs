...
pub static WALL_KICK_DATA: Lazy<Mutex<WallKickData>> = Lazy::new(|| {
    let wkd = WallKickData::new(WallKickMode::Standard);
    Mutex::new(wkd)
});
...
impl WallKickData {
    pub fn new(mode: WallKickMode) -> Self {
        match mode {
            WallKickMode::Standard => {
                let default = HashMap::from([
                    ...
                    (
                        ...
                        Vec::from(WKD_LEFT_TO_INIT_DEG180),
                    ),
...
impl Default for WallKickData {
    fn default() -> Self {
        WallKickData::new(WallKickMode::Standard)
    }
}
