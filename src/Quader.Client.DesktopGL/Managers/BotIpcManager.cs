...
public class BotIpcManager : IDisposable
{
    ...
    /*public async Task StartServerAsync()
    {
        ...
        pipeClient.StartInfo.FileName = "Quader.Bot.Client.exe";
        
        await using (var pipeServer = new AnonymousPipeServerStream(PipeDirection.InOut, HandleInheritability.Inheritable))
        {
            _logger.Info($"[SERVER] Current TransmissionMode: {pipeServer.TransmissionMode}");
            ...
            try
            {
                ...
                _logger.Info($"Sending test commands");
                string? nullCmd = null;
                await sw.WriteLineAsync("TEST1");
                line = await sr.ReadLineAsync();
                Console.WriteLine("LINE: " + line);
                await sw.WriteLineAsync("TEST2");
                line = await sr.ReadLineAsync();
                Console.WriteLine("LINE: " + line);
                await sw.WriteLineAsync("TEST3");
                line = await sr.ReadLineAsync();
                Console.WriteLine("LINE: " + line);
                //sw.WriteLine(nullCmd);

                Thread.Sleep(3000);
            }
            catch (IOException e)
            {
                _logger.Critical($"[SERVER] Error: {e.Message}");
            }
        }
        ...
        _logger.Info($"[SERVER] Client quit. Server terminating.");
    }*/
}